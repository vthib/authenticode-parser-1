...
TEST(HelperModule, byte_array_init_0)
{
    ...
    int res = ap_byte_array_init(&array, data, 10);
    EXPECT_EQ(res, 0);
...
TEST(HelperModule, byte_array_init_1)
{
    ByteArray array;
    uint8_t data[1] = {0x1};
    int res = ap_byte_array_init(&array, data, 1);
    EXPECT_EQ(res, 0);
...
TEST(HelperModule, byte_array_init_2)
{
    ...
    int res = ap_byte_array_init(&array, data, 10000);
    EXPECT_EQ(res, 0);
...
TEST(HelperModule, asn1_time_get_int64_t_0)
{
    ...
    int64_t res = ap_ASN1_TIME_to_int64_t(asn1time);
...
TEST(HelperModule, calculate_digest)
{
    ...
    int res = ap_calculate_digest(EVP_sha1(), data, 3, hash);
